...
class Assets {
        ...
        $css_content = self::concat_assets($styles, [
            self::CSS_SHEET_NAME => [
                [
                    ...
                    'attributes' => [
                        '(fonts/',
                        '(' . get_bloginfo( 'wpurl' ) . '/wp-content/themes/cmd-theme/assets/css/fonts/'
                    ]
                ],
                [
                    'name' => 'str_replace',
                    'attributes' => [
                        '(../',
                        '(' . get_bloginfo( 'wpurl' ) . '/wp-content/themes/cmd-theme/assets/'
                    ]
                ]
            ]
        ]);
    ...
    private static function include_js_libraries() {
        ...
        $scripts = [
            ...
            'slick-slider'  => '//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js',
        ];


        if (class_exists('\GFCommon')) :
            $gf_url = \GFCommon::get_base_url();

            $scripts['gform_json'] = $gf_url . "/js/jquery.json.min.js";
            $scripts['gform_gravityforms'] = $gf_url . '/js/gravityforms.min.js';
            $scripts['gform_conditional_logic'] = $gf_url . '/js/conditional_logic.min.js';
        endif;
    ...
    private static function include_css_libraries() {
        $styles = [
            'block-library' => 'wp-includes/css/dist/block-library/style.min.css',
            'slick-slider' => '//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css',
        ];
